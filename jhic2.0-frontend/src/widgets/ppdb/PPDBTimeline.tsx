      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative z-10">
        {steps.map((step) => (
          <div key={step.id} className="group flex flex-col items-center text-center">
            {/* Number Marker */}
            <div className="w-16 h-16 rounded-full bg-surface border-4 border-surface shadow-[0_0_0_4px_rgba(215,25,32,0.1)] flex items-center justify-center mb-6 text-accent group-hover:scale-110 group-hover:shadow-[0_0_0_6px_rgba(215,25,32,0.2)] transition-all duration-300 relative">
              <span className="font-extrabold text-xl">{step.id}</span>
              <div className="absolute inset-0 bg-accent/5 rounded-full z-[-1]"></div>
            </div>

            {/* Content Card */}
            <div className="bg-surface p-6 rounded-lg shadow-sm border border-border-light group-hover:-translate-y-2 group-hover:shadow-md group-hover:border-accent/30 transition-all duration-300 w-full relative overflow-hidden">
