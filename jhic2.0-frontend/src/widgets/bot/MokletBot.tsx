  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-5 right-5 z-[110] w-14 h-14 rounded-full bg-accent text-white flex items-center justify-center shadow-[0_12px_30px_rgba(215,25,32,0.4)] hover:scale-105 transition-transform"
      >
        {open ? <X className="w-6 h-6" /> : <MessageCircle className="w-6 h-6" />}
      </button>

      {open && (
        <div className="fixed bottom-24 right-5 z-[110] w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl border border-border-light bg-white shadow-2xl overflow-hidden flex flex-col">
          {/* Header */}
                ...
                <div className="flex flex-wrap gap-2">
                  {intents.slice(0, 5).map((i) => (
                    <button
                      ...
                      type="button"
                      onClick={() => say(i.label)}
                      className="px-3 py-1.5 rounded-full border border-border-light bg-white text-xs font-bold text-text-muted hover:border-accent hover:text-accent transition-colors"
                    >
                      {i.label}
                    </button>
                  ))}
          ...
          <div className="border-t border-border-light p-3 flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-border-light px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent/30"
            />
            <button
              type="button"
              onClick={() => say(input)}
              aria-label="Kirim"
              className="shrink-0 w-9 h-9 rounded-xl bg-accent text-white flex items-center justify-center hover:bg-accent-hover"
            >
...
function BotBubble({ text, typing }: { text: string; typing?: boolean }) {
  return (
    <div className="flex items-end gap-2">
      ...
      <div
        className={`bg-white border border-border-light rounded-2xl rounded-bl-sm px-3 py-2 text-sm text-text-main max-w-[80%] ${
...
function UserBubble({ text }: { text: string }) {
  return (
    <div className="flex justify-end">
      <div className="bg-accent text-white rounded-2xl rounded-br-sm px-3 py-2 text-sm max-w-[80%]">{text}</div>
    </div>
  );
}