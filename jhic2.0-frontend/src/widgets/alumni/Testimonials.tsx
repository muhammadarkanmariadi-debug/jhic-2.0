  ...
  return (
    <section className="bg-bg-main py-20 md:py-32">
      <div className="mx-auto w-full max-w-[1400px] px-4 md:px-8">
        <SectionHeader
          eyebrow="Testimoni Alumni"
          pill
          title="Kata mereka tentang Moklet"
          description="Dengarkan pengalaman langsung dari alumni kami yang telah sukses berkarier di berbagai bidang teknologi."
          className="mb-12 "
        />
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testi, idx) => (
            <figure
              key={idx}
              className="relative flex h-full flex-col rounded-[24px] border border-border-color bg-surface p-8 shadow-sm transition-all hover:-translate-y-1 hover:border-accent/30 hover:shadow-lg"
            >
              <Quote className="absolute right-6 top-6 h-12 w-12 text-neutral-100" />
              <div className="mb-4 flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="h-4 w-4 fill-amber-400 text-amber-400" />
                ))}
              </div>
              <blockquote className="mb-6 flex-1 text-[15.5px] leading-[1.7] text-neutral-700">
              ...
              <figcaption className="mt-auto flex items-center gap-4 border-t border-border-light pt-6">
                {testi.image ? (
                  <Image
                    ...
                    width={46}
                    height={46}
                    className="h-[46px] w-[46px] rounded-full bg-neutral-100 object-cover"
                  />
                ) : (
                  <div
                    className="flex h-[46px] w-[46px] items-center justify-center rounded-full text-[15px] font-extrabold text-text-inverse"
