...
export function K3Table() {
  ...
  return (
    <div className="bg-surface rounded-xl overflow-hidden border border-border-light shadow-sm">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {documents.map((doc, idx) => (
              <tr 
                key={doc.id} 
                className={`border-b border-border-light hover:bg-surface-alt transition-colors ${idx % 2 === 1 ? 'bg-surface/50' : 'bg-surface'}`}
              >
                ...
                <td className="py-4 px-6">
                  <button className="inline-flex items-center gap-1.5 bg-success hover:bg-success-strong text-text-inverse px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
