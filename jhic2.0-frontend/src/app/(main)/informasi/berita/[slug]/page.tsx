        ...
        <section className="pt-[90px] lg:pt-[110px] pb-8 px-4 md:px-8">
           <div className="relative  h-[55vh] min-h-[450px] flex items-center justify-center text-center px-6 overflow-hidden rounded-[32px] shadow-2xl">
              ...
              <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
                  <span className={`text-white text-[11px] font-extrabold tracking-widest uppercase mb-6 px-5 py-2 ${news.badgeColor} rounded-full shadow-lg`}>
                     {news.categoryLabel}
                  </span>
                  <h1 className="text-3xl md:text-5xl lg:text-[56px] font-extrabold text-white leading-[1.2] drop-shadow-xl">
                     {news.title}
                  </h1>
              </div>
           </div>
                    ...
                    <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
                       ...
                       <div className="flex flex-wrap gap-2 pt-2">
                          {['Informasi Umum', 'Prestasi', 'Agenda Sekolah', 'Pengumuman Siswa', 'Pengumuman Pegawai', 'Pengumuman Orang Tua Siswa', 'Karya Siswa'].map((cat, i) => (
                             <Link key={i} href="#" className="px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-[13.5px] font-bold text-gray-600 hover:text-accent hover:border-accent hover:bg-accent/5 transition-all">
                                {cat}
                             </Link>
                          ))}
                       </div>
                       ...
                       <div className="space-y-6 pt-2">
                          {latestNews.map((n, i) => (
                             <Link key={i} href={`/informasi/berita/${n.slug}`} className="flex items-start gap-4 group">
                                <div className="relative w-24 h-20 rounded-2xl overflow-hidden flex-shrink-0 border border-border-light">
                                ...
                                <div className="flex-1 min-w-0 pt-0.5">
                                   ...
                                   <p className="text-[12px] font-bold text-gray-400">{n.date}</p>
                                </div>
                             </Link>
                          ))}
                       </div>
